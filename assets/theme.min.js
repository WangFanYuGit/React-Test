...
$(document).ready(function() {
  $('.Anchor.slideTo').click(function () {
    let target = $(this).data('target')
    let scrollTop = $(`${target}`).offset().top - $('.header-wrapper').height()
    $("html, body").animate({
      scrollTop,
    }, 1000) 
  })
  ...
  $('.fixed_icon_wrap').css('opacity','1')
  if (window.isMobile) {
    var header_swiper = new Swiper('.fixed_icon_wrap.swiper-container', {
      loop: true,
      autoplay: true
    });
  }
  ...
  let current_index = 0;
  let timer = null;
  adaptiveShow(current_index,currentIndexAdd)
  function adaptiveShow (current_index, fn) {
    let text_item = $('#logo-rec').find(`.recommends[data-index='${current_index}']`)
    let img_item = $('#logo-rec').find(`.logo-bar__item[data-index='${current_index}']`)
    img_item.addClass('active').siblings('.logo-bar__item').removeClass('active');
    text_item.addClass('active').siblings('.recommends').removeClass('active');
    typeof fn === 'function' && fn();
  }
  function currentIndexAdd() {
    if (current_index >= 3) {
      current_index = 0
    }else {
      current_index++;
  ...
  $('#logo-rec .logo-bar__item').on('click', function() {
